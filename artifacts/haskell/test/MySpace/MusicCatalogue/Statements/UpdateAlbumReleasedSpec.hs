module MySpace.MusicCatalogue.Statements.UpdateAlbumReleasedSpec (spec) where
...
spec = do
  it "executes with arbitrary parameters" \pool ->
    property \(statementParams :: MySpace.MusicCatalogue.Statements.UpdateAlbumReleased) ->
      ioProperty do
