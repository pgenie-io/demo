module MySpace.MusicCatalogue.Statements.SelectAlbumByFormatSpec (spec) where
...
spec = do
  it "executes with arbitrary parameters" \pool ->
    property \(statementParams :: MySpace.MusicCatalogue.Statements.SelectAlbumByFormat) ->
      ioProperty do
