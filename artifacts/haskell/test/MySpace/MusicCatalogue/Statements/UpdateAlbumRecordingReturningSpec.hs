module MySpace.MusicCatalogue.Statements.UpdateAlbumRecordingReturningSpec (spec) where
...
spec = do
  it "executes with arbitrary parameters" \pool ->
    property \(statementParams :: MySpace.MusicCatalogue.Statements.UpdateAlbumRecordingReturning) ->
      ioProperty do
