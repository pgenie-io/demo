module MySpace.MusicCatalogue.Statements.SelectAlbumWithTracksSpec (spec) where
...
spec = do
  it "executes with arbitrary parameters" \pool ->
    property \(statementParams :: MySpace.MusicCatalogue.Statements.SelectAlbumWithTracks) ->
      ioProperty do
