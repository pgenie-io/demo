module MySpace.MusicCatalogue.Statements.SelectGenreByArtistSpec (spec) where
...
spec = do
  it "executes with arbitrary parameters" \pool ->
    property \(statementParams :: MySpace.MusicCatalogue.Statements.SelectGenreByArtist) ->
      ioProperty do
